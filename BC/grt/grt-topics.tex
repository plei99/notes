...
\end{bmatrix}, \qquad f = \begin{bmatrix}
    ...
    1 & 0
\end{bmatrix} \]
satisfy these relations. For a general \(\g\), these give an embedding \(\sl_2 \hookrightarrow \g\) sending the standard basis to \((e,h,f)\).
...
\begin{proof}[Proof of~\Cref{thm:Jacobson-Morozov}]
    ...
    \[ \on{tr} (\ad_h \ad_x) = 0 \]
    for any \(x \in \g^e\) because \(\ad_h \ad_x\) is strictly upper-triangular.
\end{proof}
