...
\begin{thm}[Kostant]
    ...
    \[ \g = \bigoplus_j V_j \]
    into irreducible \(\sl_2\)-representations \(V_j\) satisfies \(\dim V_j = 2 \mu_j + 1\), where \(\mu_j > 0\). Order \(V_j\) such that \(1 = \mu_1 \leq \cdots \leq \mu_k\). Then \(k = r\) and \(d_j = \mu_j + 1\).
\end{thm}

\begin{proof}
    Consider the decomposition
    \[ \g = \g^f + [\g, e]. \]
    For each \(V_j\), the lowest weight is \(-2 \mu_j\). Because \(\g^f \cap V_j\) is the lowest weight space of \(V_j\), we can let the set of \(0 \neq \xi_j \in \g^f \cap V_j\) be a basis of \(\g^f\). Define a map
