...
\section{Flag varieties and the Bruhat decomposition}%
\label{sec:Flag varieties and the Bruhat decomposition}
...
\begin{proof}[Proof of Bruhat decomposition]
    ...
    Now choose a cocharacter \(h^{\vee} \colon \C^{\times} \hookrightarrow T\) which is regular and a primitive nonzero cocharacter of \(T\). For this \(\C^{\times}\)-action, the fixed points are in bijection to the set of all Borel subgroups \(B'\) containing \(\C^{\times}\). Taking the derivative at the identity, this is in bijection with
    \[ \ab\{ \b' \mid \C h^{\vee} \subset \b' \}. \]
    Because \(h^{\vee}\) is regular, then \(\h = Z_{\g}(h^{\vee}) \subset \b'\), so we have recovered the set of all \(\b' \supset \h\).
