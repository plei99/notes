...
by considering the action of \(U\) on \(V\). To decompose, we look for the fixed subspace \(V^U\), which is the span of the highest weight vectors. Finite-dimensional representations of \(B\) are classified by \(\X^{\bullet}(B_{\on{sc}}/B_{\on{sc}}^u) = \X^{\bullet}(T_{\on{sc}}) = \Lambda\). For all \(\lambda \in \Lambda\), let \(\C_{\lambda}\) be the corresponding irreducible representation of \(B\). Then the the associated line bundle \(\mc{L}_{\lambda}\) 
\begin{align*}
    G \times^B \C_{-\lambda} &= (G \times \C_{-\lambda}) / B \\
