...
\begin{tikzcd}
    ...
    G \times E' \ar{u} \ar{r}{\Psi'} & E' \times_{B'} E' \ar{u} \ar[hookrightarrow]{r} & E' \times E'.
\end{tikzcd}
\end{equation*}
...
\begin{proof}[Proof sketch of Luna's \'etale slice theorem]
    Matsushima's criterion says that if \(H < G\) is a closed subgroup, then \(G/H\) is affine if and only if \(H\) is reductive. Now if \(G \cdot x\) is closed in \(X = \Spec A\), then \(G / G_x\) is affine and thus \(G_x\) is reductive. 
\end{proof}
