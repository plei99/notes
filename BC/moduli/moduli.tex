...
\begin{defn}
    ...
    \begin{enumerate}
        \item The diagonal morphism \(\Delta_{\mf{X}} \colon \mf{X} \to \mf{X} \times \mf{X}\) is representable by algebraic spaces, quasi-compact, and separated;\footnote{Some authors (like the Stacks project) remove the separation conditions.}
        \item There exists a surjective \'etale (resp. smooth) morphism \(U \to \mf{X}\) where \(U\) is a scheme.
    \end{enumerate}
\end{defn}

\begin{defn}
    The quotient stack \([X/G]\) and the morphism \(X \to [X/G]\) is a smooth atlas (at least in characteristic zero) and an algebraic space if \(G\) is finite and all stabilizers are trivial.
\end{defn}

\begin{exm}
    If we define \(\mc{M}_g\) to send a scheme \(B\) to the \textbf{set} of isomorphism classes of genus \(g\) smooth curves over \(B\), then it is not a stack. For example, consider a genus \(g\) curve \(C\) with a nontrivial involution and an \'etale double cover \(B' \to B\). Then we can consider the quotient \(X\) of \(C \times B'\) by the diagonal action of \(\Z_2\), and the resulting diagram
    \begin{equation*}
    \begin{tikzcd}
