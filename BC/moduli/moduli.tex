...
\begin{exm}[Non-proper action, Mumford]
    Let \(G = \on{SL}(2)\) and
...
where \(\Z_2\) acts by swapping the two origins. This stack does not have a good moduli space which is a scheme, but it does have a good moduli space which is a non-separated algebraic space by Keel-Mori.
...
\begin{defn}
    Let \(\mf{X}\) be an algebraic (resp. DM) stack. A morphism \(\phi \colon \mf{X} \to X\) to an algebraic space \(X\) is a \textit{good (resp. coarse) moduli space} if
    \begin{enumerate}
        \item \(\phi_* \colon \ms{QCoh}(\mf{X}) \to \ms{QCoh}(X)\) is exact (namely \(\phi\) is \textit{cohomologically affine});
        \item For every algebraically closed field \(k\), \(\phi\) induces a bijection from closed \(k\)-points in \(\mf{X}\) to \(X(k)\).\footnote{If \(\mf{X}\) is DM, then all points are closed, so this condition is equivalent to \(\phi\) inducing a bijection from \(\mf{X}(k)\) to \(X(k)\).}
    \end{enumerate}
\end{defn}

\begin{rmk}
    If \(\phi\) exists, then it is universal among maps to algebraic spaces and hence unique.
\end{rmk}
