...
\begin{exm}
    ...
    The reason it is called a category fibered in groupoids is that there is a morphism to \(\mc{Sch}_S\) and each \(p^{-1}(B) = F(B)\) is a set (which is the simplest example of a groupoid).
\end{exm}
...
\begin{defn}
    A \textit{stack} is a category \(\mc{C}\) fibered in groupoids over \(\mc{Sch}_S\) if
    \begin{enumerate}
        \item For all \(B \in \ms{Sch}_S\), the Isom functor \(\ms{Isom}(X, Y)\) is a sheaf on the big \'etale site of \(B\);
        \item Descent is effective in the \'etale topology for all \(B\).
    \end{enumerate}
\end{defn}
