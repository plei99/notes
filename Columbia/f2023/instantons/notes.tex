...
\section{Gauge-theoretic instanton counting}
...
\subsection{Mathematical realization}
...
Therefore, $T_{\ul{\lambda}} \ol{\mc{M}}_k^{\mr{framed}}(N)$ is the cohomology of a complex
\[ \End(\C^k) \to \End(\C^k) \oplus \End(\C^k) \oplus \Hom(\C^N, \C^k) \oplus \Hom(\C^k) \oplus \C^N \to \End(\C^k), \]
where the two arrows are given by
\begin{align*}
    d_1(\phi) &= ([B_1, \phi], [B_2, \phi], -\phi I, 0) \\
    d_2(\delta B_1, \delta B_2, \delta I, \delta J) &= [B_1, \delta B_2] + [\delta B_1, B_2] + I \delta J.
\end{align*}
