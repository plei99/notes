...
\[
Z(\vec{a},\vec{m}, \vec{q}, \epsilon_1,\epsilon_2)
\]
dependent on many parameters. The parameters $\epsilon_i$ were special,
...
\[
\vec{\lambda} = \qty{a^{(i)}_\alpha + \epsilon_1(I - 1) + \epsilon_2(J-1) \mid i \in V_\Gamma, \alpha = 1,\dots,n, (I,J) \in \lambda^{(i,\alpha)}}.
\]
...
Then we could be interested in the limiting behaviour as $\epsilon_1,\epsilon_2 \to 0$. In this probablistic case that would 
require us to find the limiting measure. 
We can draw a curve which describes the boundary of these finite sets, 
and seek its limiting shape as $\epsilon_1,\epsilon_2 \to 0$. 

Physically, $\epsilon_1,\epsilon_2$ effectively confine instantons into a polydisc of volume $\approx (\epsilon_1 \epsilon_2)^{-1}$.
Then taking the limit $\epsilon_1,\epsilon_2 \to 0$ is a \emph{thermoydynamic limit}. 
We view $Z = exp( \epsilon_1^{-1} \epsilon_2^{-1} \mathcal{F}(\vec{a},\vec{m},\vec{q}) + \dots)$,
and our goal is to extract the function $\mathcal{F}$. 
The typical size of a partition will be 
\[
|\lambda^{(i,\alpha)}| \sim (\epsilon_1 \epsilon_2)^{-1} \#(\vec{a},\vec{m},\vec{q}).
\]
...
weights of the symmetry group therefore split in pairs. So if $w$ appears in $T \overline{\mathcal{M}}_k(n)$, so does $\epsilon_1 + \epsilon_2 - w$. 
...
\]
If we specialize $\epsilon_1 + \epsilon_2 = 0$, then 
\[
\mu_\lambda = \frac{\prod(\dots)}{\prod (-w^2)}.
\]
Each $w$ is a linear function of the $\vec{a},\vec{\epsilon}$ terms. Choosing $\epsilon_1 \in i \mathbb{R}, a_\alpha^{(i)} \in i \mathbb{R}$, 
...
\[
a_\alpha^{(i)} - a_\beta^{(j)} \in \epsilon_1 \mathbb{Z}_+ + \epsilon_2 \mathbb{Z}_+,
\]
...
\[
c_\square = \epsilon_1(I- 1) + \epsilon_2(J-1).
\]
Then 
\[
...
\begin{align*}
    ...
    &+ \sum_i \rho_i(x) [ \frac{x^2}{2} \log q_i + \sum_{f= 1}^{m_i} K(x - \mu_f^{(i)})],
\end{align*}
wherein 
\[
...
\begin{exm}
  ...
  Then $g = diag(z_1 \frac{Y_1}{P_1},\dots, z_r \frac{Y_r}{Y_{r-1}'}, z_{r+1} \frac{P_r}{Y_r}) \in GL(r+1)$.
