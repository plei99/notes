...
to teach everyone about the Dirac operator. Finally, I would like to thank Google for Gemini 3 Pro allowing me to upscale the original hand-drawn figures to Tikz.
...
\begin{proof} For simplicity, suppose $f, g$ only differ on $\sigma^n$. Given
    $e^{n+1}$, we want to show that \[ c_g(e) - c_f(e) = [e:\sigma]
        d_{f,g}(\sigma). \] Up to homotopy, we may assume that in Figure~\ref{fig:shaded}, the shaded
            ...
            components of the shaded part is $[e:\sigma]$.
            \begin{figure}[htpb]
            \begin{center}
...
\begin{proof} Note that $M^{n-1} \cong M \setminus \qty{\text{finite number of
    ...
    $y \mapsto v(y)$. The local degree behaves like in~\Cref{fig:locdeg} 
        \begin{figure}[htpb]
        \begin{center}
        \begin{tikzpicture}[scale=1, transform shape, thick,
