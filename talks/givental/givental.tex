...
\begin{rmk}
    ...
    \[ S_X^*(-z) S(Z) = 1. \]
    Using this formalism, the $J$-function is given by $S^X(t,z)^* \mbf{1} = z^{-1} J_X(t,z)$.
\end{rmk}
...
\[ \partial_0^1 \mc{F}(t) = \frac{1}{2} (t_0, t_0) + \sum_{n=0}^{\infty} \sum_{j} t_{n+1}^{\nu} \partial_n^{j} \mc{F}(t) \]
...
\[ \mc{F}_g \coloneqq \sum_{\beta, n} \frac{Q^{\beta}}{n!} \ab<t(\psi), \ldots, t(\psi)>_{g,n,\beta}^X \]
and the \textit{total descendent potential}
\[ \mc{D} \coloneqq \exp\ab(\sum_{g \geq 0} \hslash^{g-1} \mc{F}_g). \]
