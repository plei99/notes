...
Consider a family over a disk $\Delta_t$. We have two divisors $(\varphi_1 = 0)$ and $(\varphi_2 = 0)$, but they may meet on the central fiber. We can blow up the intersection point to separate the two divisors as in~\Cref{fig:blowup1}. Denote the blowup morphism by $\pi \colon \tilde{ \mc{C} } \to \mc{C}$. Then we actually have
...
Now $\pi^* \varphi_2$ vanishes to order $1+c$ on $E$. Blowing up again, the vanishing orders now become $(1,2c) \sim (0,2c-1)$, which seems worse. However, this can be resolved combinatorially to obtain a stable limit. See~\Cref{fig:blowup2} for an example.
\begin{figure}[htpb]
\begin{center}
\begin{tikzpicture}[scale=1, transform shape]
    \draw[thick] (0,0) rectangle ++(2,2);
...
\begin{rmk}
    ...
    is expected to hold for all of the examples in~\Cref{tab:oneparameter}. It has been proven for hypersurfaces in weighted projective space by the author~\cite{polynomiality}.
\end{rmk}
...
\begin{exm}
    ...
    \begin{tikzpicture}[scale=1, transform shape]
        ...
        \draw[-,thick] (H) -- (I);
        \draw[every loop/.style={-},thick] (H) edge[loop right]  (I);
        \draw[every loop/.style={-},thick] (I) edge[loop left]  (H);
