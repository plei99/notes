...
Note that the first condition means that for any morphism \(T \to \mf{X} \times \mf{X}\) from an algebraic space, \(T \times_{\mf{X} \times \mf{X}} \mf{X}\) is an algebraic space. This is equivalent to all Isom sheaves being algebraic spaces and implies that any morphism from a scheme to \(\mf{X}\) is representable.
...
\begin{exm}
    Let \(\mf{X}\) be a smooth separated DM stack of finite type over \(\C\). If either the generic stabilizer of \(\mf{X}\) is trivial or the coarse moduli space of \(\mf{X}\) is quasi-projective, then \(\mf{X}\) is a global quotient. Here, a DM stack is an algebraic stack with unramified diagonal, and a global quotient means a stack of the form \([U/\on{GL}_n]\) for some algebraic space \(U\) with an action of \(\on{GL}_n\).
\end{exm}
...
Even these do not always exist. A good moduli space is a different weakening: it replaces the requirement of a bijection on geometric points by a cohomological affineness condition.
...
\begin{thm}[Virtual non-abelian localization]
...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item Each function \(\mu_{\gamma}\) is strictly quasi-concave, namely for all \(\delta \in [0,1]\) and linearly independent \(x_0, x_1 \in \R^q\) such that \(\mu(x_0), \mu(x_1) \geq 0\), we have
...
\begin{thm}
    ...
    \begin{itemize}
        \item For any DVR \(R\) with fraction field \(K\) and residue field \(k\), we have \(\mu(f_K^{\mr{HN}}) \leq M^{\mu}(f_k)\) (HN-specialization);
...
\begin{thm}[Beilinson-Bernstein-Deligne-Gabber]
...
For a Deligne-Mumford stack which is not assumed to be smooth, there are two versions
\[ K_0(X) \supset K^0(X), \]
...
Using the action of \(S_n\) on \(\bar{M}_{g,n}(X, \beta)\) given by permuting the marked points, the objects \(\bigotimes_{i=1}^n \ev_i^* V\) and \(\bigotimes_{i=1}^n L_i^k\) are \(S_n\)-equivariant.
...
\begin{exm}[Second toy example]
    ...
    \begin{enumerate}
        \item For all \(x \in C^{\mr{sm}}\), we have
...
Because \(X\) is Calabi-Yau, the moduli space \(\Mbar_{g,n}(X,\beta)\) has virtual dimension zero, and the string equation gives \(\ab<\1, \gamma_2, \ldots, \gamma_n>_{g,n,\beta} = 0\) unless \((g,n,\beta) = (0,3,0)\). Therefore, we define the primary invariants by
...
We see that every column of~\Cref{tab:quinticbps} eventually vanishes. This is the Castelnuovo bound, which for the quintic states that \(n_{g,d} = 0\) once \(g > \frac{d^2+5d+10}{10}\) and was proved by Liu-Ruan. Integrality of the entries is the Gopakumar-Vafa conjecture, which was proved by Ionel-Parker.

In genus zero, Candelas-de la Ossa-Green-Parkes computed \(F_0\) using period integrals on the mirror. In higher genus, Bershadsky-Cecotti-Ooguri-Vafa gave a recursion which produces \(F_g\) from the \(F_h\) with \(h < g\) up to finitely many unknown constants. Our goal is to explain how log GLSM determines this ambiguity.
...
Recall that for the quintic we consider the \(I\)-function
\[
...
We can interpret this as a sum over stable graphs. A vertex \(v\) contributes the lower-genus correlator \(P_{g_v, \on{val}(v)}\); an internal edge contracts two of the insertions against a propagator; the legs record the remaining insertions. Each graph is weighted by \(\frac{1}{\ab|\Aut \Gamma|}\). As usual
...
\begin{rmk}
    The BCOV graph rule implies the anomaly equation, but the anomaly equation by itself does not determine the graph rule. To recover the same solution, we also need the propagator convention, the lower-genus vertex data, and the boundary conditions.
\end{rmk}
...
\begin{rmk}
    ...
    parameters remain and are genuinely new effective invariants. This is exactly the number of \(\infty\)-vertex unknowns predicted by~\Cref{eqn:balancing condition}, which is the number of \(\beta\) with \(\beta \leq \frac{2g-2}{5}\).
\end{rmk}
...
the individual twisted graph contributions involve extra generators. The key point is that these cancel in the above linear combination and leave exactly the finite generation variables. Writing \(X = \mc{X}_1\) and \(Y_1 = \mc{Y}_1\), we obtain
\begin{align*}
...
and these coefficients are the Gromov-Witten invariants \(N_{2,d}\).
...
\end{align*}
Note that \(Y = L^5\) is the discriminant generator while \(\mc{Y}_1\) is a logarithmic derivative generator.
...
which means that every intermediate negative power \(T_c^{-(2g-3)}, \ldots, T_c^{-1}\) vanishes. This is \(2g-2\) conditions and is not implied by finite generation.
...
For fixed \(\vec{a}\) and \(\vec{\delta}\), individual terms in the \(\psi\)-expansion may have poles, but they cancel in the complete rational descendant insertion before the rest graph sum over \(\vec{a}\) is taken.
...
Unfortunately, for \(p \geq 2\) the toric graph of \(X_p\) is not convex, so \(X_p\) is not semiprojective. This is the reason the case at hand is not covered by the existing literature, as we will explain in~\Cref{sub:Topological recursion and the remodeling conjecture}.
...
\label{prop:R matrices coincide}
    The two \(R\)-matrices coincide: \(R(z) = \check{R}(u)|_{u=-z}\).
\end{prop}
