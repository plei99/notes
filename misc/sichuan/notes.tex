...
Because \(X\) is Calabi-Yau, the moduli space \(\Mbar_{g,n}(X,\beta)\) has virtual dimension zero, and the string equation gives \(\ab<\1, \gamma_2, \ldots, \gamma_n>_{g,n,\beta} = 0\) unless \((g,n,\beta) = (0,3,0)\). Therefore the primary theory is carried by the numbers
...
Every column of~\Cref{tab:quinticbps} eventually vanishes. This is the Castelnuovo bound, which for the quintic states that \(n_{g,d} = 0\) once \(g > \frac{d^2+5d+10}{10}\) and was proved by Liu-Ruan. Integrality of the entries is the Gopakumar-Vafa conjecture, which was proved by Ionel-Parker.
...
The BCOV recursion is a Feynman rule, and it comes from a finite-dimensional model for the quantization of a symplectic transformation as in~\Cref{sub:Operator formalism and geometric quantization}. Let \(\mc{H} = \mc{H}^- \oplus \mc{H}^+ \subset \mc{H}_{\infty}\) be symplectic with \(d+1 = \dim \mc{H}^+ = \dim \mc{H}^-\) and set
\[ R = \begin{pmatrix} A & B \\ C & D \end{pmatrix} \in \End \mc{H}, \qquad \mc{Q}(\mbf{x},\bp) = (\bp, D^{-1}\mbf{x}) - \frac{1}{2}(\bp, D^{-1}C\bp). \]
Given a total potential \(F(\hbar, \mbf{x})\), the transformed potential is defined formally by the Gaussian integral
...
The graphs here are stable graphs, as in~\Cref{sub:R-matrix action}. A vertex \(v\) contributes the lower-genus correlator \(P_{g_v, \on{val}(v)}\), an internal edge contracts two of the insertions against a propagator, the legs record the remaining insertions, and the graph is weighted by \(\frac{1}{\ab|\Aut \Gamma|}\). As usual
...
The recursion can also be written as a differential equation. The B-model amplitudes \(\mc{F}_g(z,\bar{z})\) are not holomorphic, and for \(g \geq 2\) their failure to be holomorphic is given by
...
\begin{rmk}
    The implication only goes one way. The BCOV graph rule implies the anomaly equation, but the anomaly equation by itself does not determine the graph rule, because to recover the same solution one also needs the propagator convention, the lower-genus vertex data, and the boundary conditions.
\end{rmk}
...
\begin{itemize}
    ...
    \item \textit{Quintic Gromov-Witten theory} is obtained by setting \(\mbf{C} = \mbf{C}^{\mr{eff}}\) and then letting \(\lambda \to 0\), and is what we want.
\end{itemize}
...
where \(\rho\) forgets the map and stabilizes the domain. After shifting by \(\tau(q)H\) with \(\tau(q) = I^{\lambda}_1(q)/I^{\lambda}_0(q)\), the shifted theory \(\Omega^{\lambda,\tau}\) is semisimple, and after extending scalars to \(\Q(\zeta_5,\lambda)\) the idempotent basis satisfies
\[ \omega^{\lambda}_{g,n}(e_{\alpha_1}, \ldots, e_{\alpha_n}) = \sum_{\alpha}\Delta_{\alpha}^{g-1}\prod_i \delta_{\alpha,\alpha_i}. \]
Therefore~\Cref{thm:teleman} applies and gives
...
\begin{rmk}
    The edge factor has no pole. Setting \(z_2 = -z_1\) and using the symplectic condition in the form \(R^{-1}(z) = R^*(-z)\), the numerator vanishes, so it is divisible by \(z_1+z_2\) and \(V_R\) is a power series in the two cotangent line variables. The coefficient of \(z_1^p z_2^q\) is the class placed at a node with \(\bar{\psi}_1^p \bar{\psi}_2^q\), and contracting it against the two adjacent topological field theory tensors glues the branches.
\end{rmk}

\subsection{How to compute the \(R\)-matrix}%
\label{sub:How to compute the R-matrix (Guo)}

The genus zero input is the formal equivariant \(I\)-function
...
\begin{rmk}
    A fixed genus only needs finitely much of \(R\) for dimensional reasons. On \(\Mbar_{g,n}\) only classes of total degree at most \(3g-3+n\) contribute and everything else integrates to zero, so for an unmarked potential we need only \(R_0, \ldots, R_{3g-3}\). Genus \(4\) stops at \(z^9\) and genus \(5\) at \(z^{12}\).
\end{rmk}

\subsection{Genus two}%
\label{sub:Genus two}

We will now carry out both routes in genus two. The geometric route uses the \(t\)-twisted stable quotient theory, applies virtual localization to get five contributions, and then applies stable quotient wall crossing to land on quintic Gromov-Witten theory. The formal route uses the semisimple \(\lambda\)-twisted theory and the \(R\)-matrix to compute a stable graph sum, and gives only the formal leading part, so the effective and meromorphic rest graphs must still be added.
...
\begin{notn}
    ...
    where \(\mc{L}\) is the universal stable quotient line bundle and \(c_1^{\C^{\times}}([-1]) = -t\). This is the stable quotient sign convention, and is not the same as the \([1]\) appearing in the stable map cycle above.
\end{notn}
...
the individual twisted graph contributions involve extra generators, but these cancel in the above linear combination and leave precisely the finite generation variables. Writing \(X = \mc{X}_1\) and \(Y_1 = \mc{Y}_1\), we obtain
\begin{align*}
...
For the formal route, the Givental-Teleman formula for \(\Omega^{\lambda}_{2,0}\) is a sum over the seven stable graphs of genus two with no legs, which are drawn in~\Cref{fig:g2stable}. Only \(R^{-1}(z)\) through \(z^3\) is needed, and the translations expand the seven topologies into \(17\) decorated strata.
...
\begin{rmk}
    These are dual graphs of stable curves and are not the graphs of~\Cref{fig:tropgraphs}, which are the rigid tropical types obtained by applying the tropical decomposition formula to the reduced theory of the log GLSM. The two lists of topologies happen to coincide here, but a vertex means something different in each: here it is a component of the domain curve carrying a topological field theory tensor, while there it is a level of a bipartite tropical curve carrying either a log or an \(\infty\)-vertex moduli problem.
\end{rmk}
...
\end{align*}
This is the exact formal \(\lambda\)-twisted answer and is not \(F_2^{\mr{GW}}\). Note also that \(Y = L^5\) is the discriminant generator while \(\mc{Y}_1\) is a logarithmic derivative generator.
...
\begin{thm}[Graded finite generation]%
...
\begin{thm}[Holomorphic anomaly equation for the extended family]
    We have
...
The proof is the cutting argument of~\Cref{sub:The holomorphic anomaly equation}, which works because the derivative of the \(R\)-matrix propagator is simply
...
Derivatives of \(R^{-1}\) and of every edge remain inside the same graded ring, so cutting a differentiated edge closes up on lower-genus correlators in \(\mc{R}\). Setting \(\mbf{C}=0\) recovers the same equations for the formal \(\lambda\)-twisted theory.

\subsection{Boundary conditions}%
\label{sub:Boundary conditions}

There are two sources of conditions on the holomorphic ambiguity.

The first is regularity at the orbifold or Landau-Ginzburg point \(q = \infty\), equivalently \(L \to 0\). For either the formal or an assembled extended potential \(F_g^{\star}\), set
...
where \(I_0^{\mr{co}}, I_1^{\mr{co}}\) is a local conifold period pair normalized so that the vanishing flat coordinate satisfies \(T_c = 0\) at the conifold. Given a rational expression \(f\) in the finite generation data, its conifold value \(f^{\mr{co}}\) is obtained by substituting the Laurent expansions of all generators as series in \(T_c\). \Cref{thm:graded finite generation} gives the pole bound
...
\begin{exm}
    ...
    and the2^{\lambda,\mr{co}} = \frac{1}{240T_c^2} + \f\(c{31\) 000} - \fras two gap and not merely the pole order bound.
\end{exm}
...
For fixed \(\vec{a}\) and \(\vec{\delta}\), individual terms in the \(\psi\)-expansion may have poles. The assertion is about the complete rational descendant insertion, whose negative powers cancel before the rest graph sum over \(\vec{a}\) is taken, and it is a statement about these specific formal correlators rather than about every unassembled meromorphic correlator.

\Cref{conj:formal conifold gap,conj:special correlator regularity} transfer the gap to quintic Gromov-Witten theory. A rest graph contribution has the shape
...
and the \(C^{\infty}\) factors are finite polynomials in \(q\), hence conifold regular. Therefore the formal leading gap together with regularity of the special correlators implies the gap for the extended family, and hence for the quintic.
...
Altogether, the computation runs from Picard-Fuchs data to the Givental \(R\)-matrix and the operator \(S_{\tau}(z)\), then to formal stable graphs and meromorphic rest graphs, then to the holomorphic anomaly equation and finite generation, and finally to the orbifold and conifold boundary conditions, which determine \(F_g^{\mr{GW}}\).
...
Localizing at these two fixed points expresses the invariants in terms of equivariant cohomology, where \(H^*_{\T'}(\mr{pt}) = \C[\lambda_1,\lambda_2]\).

Note that for \(p \geq 2\) the toric graph of \(X_p\) is not convex, so \(X_p\) is not semiprojective. This is the reason the case at hand is not covered by the existing literature, as we will explain in~\Cref{sub:Topological recursion and the remodeling conjecture}.
...
so~\Cref{conj:cgmps} is equivalent to the statement that \(b_g = a_g\) for all \(g \geq 2\).

\subsection{The main results}%
\label{sub:The main results (Guo local P1)}

The global mirror symmetry picture of~\Cref{sec:Structure of higher-genus GW invariants of the quintic (Shuai Guo)} applies here as well, and the three main theorems describe the behavior of \(F_g^{X_p}\) at each of the boundary points of the one-dimensional B-model moduli space. The limit \(q \to 0\) is the large complex structure limit and gives the Gromov-Witten expansion in the A-model variable \(Q\). At \(q \to 1\) the framed \(R\)-matrix shows that \(F_g\) remains regular. The interesting point is \(q \to \frac{1}{p^2}\), which is a higher-order critical point: unlike an ordinary conifold singularity, whose genus-\(g\) leading pole has order \(2g-2\), this degeneration has pole order at most \(5g-5\), and its leading coefficient is governed by the \((3,2)\) minimal model.
...
This validates the structural ansatz~\Cref{eqn:cgmps ansatz}.
...
Because \(X_p\) is noncompact, the moduli space \(\Mbar_{g,0}(X_p,d)\) is not proper and the degree of the virtual class is not defined a priori. The standard remedy is to introduce a torus action and define the invariants by virtual localization, and we will do this by passing to a twisted theory on \(\P^1\).

Choose a framing \(f \in \Z\) and let \(\T_f \coloneqq \ker(\lambda_2 - f\lambda_1)\). The two localized weight rows, each of which includes one zero weight, are
...
This theory is a CohFT \(\Omega^{\lambda,\mf{f}}\) with state space the equivariant cohomology ring
\[ H^*_{\T_{\mf{f}}}(\P^1) = \frac{\C[H,\lambda]}{(H+\lambda)(H-\lambda)} \]
and pairing
\[ (a,b) \coloneqq \int_{\P^1} a \cup b \cup e^{-1}_{\T_{\mf{f}}}(\mc{O}(p-1)\oplus\mc{O}(-p-1)). \]
In the flat basis \(\ab\{\1,H\}\) the Gram matrix is
...
where \(\st\) is the forgetful map to \(\Mbar_{g,n}\) and the twisted virtual class is
...
\begin{rmk}
    The degree-positive invariants \(N^{X_p}_{g,d\geq1}\) are independent of \(\mf{f}\) by the topological vertex, and their \(\lambda\)-independence also follows from~\Cref{thm:local P1 reconstruction}. We will use the \(\mf{f}\)-independence in the proof of~\Cref{thm:local P1 rationality}.
\end{rmk}
...
The eigenvalues \(L_{\alpha}\) of this matrix are generically distinct, so \(\Omega^{\lambda,\mf{f}}\) is semisimple. We write \(\ab\{e_{\alpha}\}\) for the idempotent basis, \(\Delta_{\alpha}^{-1} = (e_{\alpha},e_{\alpha})\) for the lengths, and \(\bar{e}_{\alpha} \coloneqq \Delta_{\alpha}^{1/2}e_{\alpha}\) for the normalized canonical basis.

By~\Cref{thm:teleman}, we have
\[ \Omega^{\lambda,\mf{f}} = R.T_R.\ab(\bigoplus_{\beta=0}^1 \Omega^{\mr{KW}}_{\beta}), \]
where \(\Omega^{\mr{KW}}_{\beta}\) is a one-dimensional trivial CohFT and \(T_R(z) = z(\wt{\1}-R^{-1}(z)\1)\) with \(\wt{\1} \coloneqq \sum_{\beta}\bar{e}_{\beta}\). The \(R\)-matrix is obtained from the quantum differential equation
...
where the initial condition \(C_{\alpha}(z)\) is an exponential involving Bernoulli numbers. Writing \(R_{i\bar{\alpha}}(z) \coloneqq (R^*(z)H^i, \bar{e}_{\alpha})\), the quantum differential equation becomes the second-order equation
...
The Eynard-Orantin topological recursion takes as input a \textit{spectral curve} \((\Sigma,x,y,B)\), where \(\Sigma\) is a Riemann surface, \(x\) is locally defined with meromorphic differential \(\d{x}\) having finitely many simple zeros, \(y\) is holomorphic near those zeros with \(\d{y} \neq 0\) there, and \(B\) is a fundamental bidifferential on \(\Sigma \times \Sigma\). Near a zero \(z^{\beta}\) of \(\d{x}\), let \(\bar{z} \neq z\) be the local conjugate point with \(x(\bar{z}) = x(z)\) and define the recursion kernel
\[ K_{\beta}(z_0,z) \coloneqq \frac{\int_{\bar{z}}^z B(z_0,z')}{2(y(z)-y(\bar{z}))\d{x}(z)}. \]
Setting \(\omega_{0,0} = \omega_{1,0} = 0\), \(\omega_{0,1}(z) = y(z)\d{x}(z)\), and \(\omega_{0,2}(z_1,z_2) = B(z_1,z_2)\), the multi-differentials are defined recursively for \(2g-2+n+1 > 0\) by
\begin{align*}
...
and the analogous transform of \(\d{y}\) defines \(T(u)\), which equals \(u(\wt{\1}-\check{R}^{-1}(u)\1)\) when the resulting CohFT has flat unit. Then \(\check{R}\) is symplectic, the curve CohFT is \(\check{\Omega} = \check{R}.T.\ab(\bigoplus_{\beta}\Omega^{\mr{KW}}_{\beta})\), and writing \(\d{\zeta^{\bar{\beta}}_k} \coloneqq \ab(-\d{} \circ \frac{1}{\d{x}})^k \d{\zeta^{\bar{\beta}}}\), the correspondence reads
\begin{align*}
...
\end{align*}
Solving \(\d{x_{\mf{f}}} = 0\), the critical points are
...
and computing the inner products \(\eta(e_{\alpha},e_{\beta}) = \delta_{\alpha,\beta}\frac{y'(z^{\alpha})^2}{x''_{\mf{f}}(z^{\alpha})}\) yields lengths \(\check{\Delta}_{\alpha}\) which satisfy \(\check{\Delta}_{\alpha} = \Delta_{\alpha}\). Therefore the Frobenius structure of the spectral curve is the quantum product of \(X_p\).
...
are exactly the entries of the quantum product computed above, so these agree with the quantum differential equation for the Gromov-Witten \(R\)-matrix. For the initial conditions, taking the limit \(q \to 0\) of the Laplace transforms along the thimbles produces Beta functions, hence products of Gamma functions, and Stirling's expansion matches the Bernoulli-number initial condition \(C_{\alpha}(z)\).
...
In~\Cref{prop:R matrices coincide} the canonical lengths agree, and after the substitution \(u = -z\) the genus-\(g\) stable graph sums differ by the overall parity \((-1)^{3g-3} = (-1)^{g-1}\), which gives~\Cref{thm:local P1 mirror symmetry}.

\subsection{Rationality}%
\label{sub:Rationality}

Because the higher-genus potential is independent of \(\mf{f}\), we may set \(\mf{f} = 0\), which simplifies both the canonical basis and the quantum differential equation. The latter becomes
...
so~\Cref{thm:local P1 rationality} will follow once we know that the degree of the numerator in \(\Delta\) is bounded by \(5g-5\). Since \(\Delta\) has a simple pole at \(q=1\), it suffices to prove that \(F_g^{X_p}\) is regular at \(q=1\), because the numerator must then cancel the highest powers of \(\Delta\). Regularity at \(q=1\) follows from the \(\mf{f}\)-independence of \(F_g^{X_p}\) together with the existence of the limit \(q \to 1\) of the \(R\)-matrix for \(\mf{f} \neq 0\).

\subsection{The limit \(\Delta = 0\)}%
\label{sub:The limit Delta = 0}

Evaluating \(\mc{P}_g(\Delta,p^2)\) at \(\Delta = 0\) is precisely the double-scaling limit. Rescale the canonical lengths by
...
The \(p\)-dependent factors in~\Cref{lem:R matrix at Delta zero} now factor out and leave purely rational coefficients. Construct the topological field theory \(\Omega^{\mr{top}}\) on \(V = \on{span}\ab\{\phi_0,\phi_1\}\) with unit \(\phi_0\), where
...
with flat coordinate \(\tau^1 = \ep^4/2\). Setting \(\tau^0 = 0\) and \(\ep = 1\) and identifying \(\phi_0 = \partial_{\tau^0}\) and \(\phi_1 = \partial_{\tau^1}\), the \(R\)-matrix obtained from \(X_p\) at \(\Delta = 0\) coincides with the \(R\)-matrix of this curve. Therefore \(C_g\) is exactly the free energy of the \((2,3)\) minimal model.
