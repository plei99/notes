...
representation theorem. Further topics depending on instructor.  \end{abstract}
    ...
    \begin{defn}[Sigma-Algebra] A $\sigma$-algebra $\Sigma$ to be a collection
    ...
    \begin{defn}[Sigma-finite] A measure $\mu$ is $\sigma$-finite if $\mu(X) =
...
\subset B$ implies $A \in \Sigma$.  \end{defn}
    ...
    \begin{defn}[Measurable function] A function $f: X \rightarrow R$ is
    measurable if $f^{-1}(\alpha, \infty)$ is measurable for all real $\alpha$.
\end{defn}

    Given the notion of a measurable function, we can integrate:

    \begin{defn}[Simple Function] A simple function is a finite linear
...
\end{lem}
    ...
    \begin{lem}[Convergence theorems] If $\mu_n \rightarrow \mu$ setwise on
    ...
    \begin{defn}[Mutually Singular] Two measures $\mu, \nu$ are mutually
...
$X = A \bigcup B$ such that $\mu(B) = \nu(A) = 0$.  \end{defn}
    ...
    \begin{defn}[Absolute Continuity] Given two measure $\mu, \nu$, $\nu$ is
...
that $\nu = \mu_f$. We write $f = \frac{d\nu}{d\mu}$.  \end{thm}
    ...
    \subsection{Lecture 2 (Jan 24)}% \subsubsection{Inanities}% Robin came in
   ...
   \subsubsection{Review Continued}% Recall that we have a monotone increasing
   ...
   Let $J$ be the set of jumps. Then \[ F_j = \sum_{x_i \in J} (F(x_i+) -
    ...
    Recall again the Radon-Nikodym Theorem: \begin{thm}[Radon-Nikodym] Let $(X,
    ...
    Now recall signed measures: \begin{defn}[Signed Measure] A signed measure
...
and does not take on both positive and negative infinity.  \end{defn}
    ...
    \subsection{Lecture 2 continued}% Our first issue: are products of
    measurable sets measurable? What is the correct product $\sigma$-algebra?
    We will simplify the structure.

    \begin{defn}[Semi-algebra] A semi-algebra $\mathcal{C}$ satisfies: if $A,B
...
Y} f \d(\mu\times\nu)$.  \end{enumerate} \end{thm}
    ...
    \begin{exm}[Counterexample if non-negativity is not required] Consider $f =
...
can extend to $X$ by passing to the limit.  \end{rmk}
    ...
    \begin{exm}[Heat equation] The heat equation is $\Delta_t = \Delta u$. This
...
\end{thm}
    ...
    \begin{exm}[Equivalence of metrics] Let $X$ be a positive definite
    ...
    \begin{defn}[Sequentially Compact] Let $X$ be a metric space. $K \subset X$
    is sequentially compact if every sequence $\{x_k\} \subset K$ has a
convergent subsequence with limit in $K$.  \end{defn}

    \begin{thm}[Heine-Borel] $K \subset \R^d$ is sequentially compact if and
    ...
    \begin{defn}[Totally Bounded] A set $A$ is totally bounded if and onlg if
...
>\ep$, so this sequence has no Cauchy subsequence.  \end{proof} \end{thm}
    ...
    \begin{defn}[(Topological) Compactness] $K$ is compact if every open cover
...
\end{proof}
    ...
    \begin{thm}[Weierstrass approximation theorem] Any function $f \in
    ...
    \begin{defn}[Uniform Equicontinuity] $\mathcal{F}$ is uniformly
...
\geq \ep / n$. This contracts equicontinuity.  \end{proof} \end{thm}
    ...
    \begin{thm}[Arzela-Ascoli (1880s?)] Let $K$ be compact. A subset
...
\end{enumerate} \end{proof} \end{thm}
    ...
    \begin{exm}[Not equicontinuous] Let $h_n$ be the skew hat function, which
...
\end{exm}
    ...
    \begin{exm}[Existence theorem for ODEs] Consider the ODE $\dot{u} =f(t,u)$
...
pairs of elliptical orbits.  \end{exm}
    ...
    \subsection{Lecture 9 (Feb 21)}% Last time we discussed Peano's proof of
...
``Gronwall's Lemma.''
 ...
 0$ for all $t$. \qedhere \end{description}           \end{proof} \end{thm}
    ...
    \begin{proof}[Alternative existence proof for differential equations] This
        ...
        \begin{defn}[Strict Contraction] A nonlinear map $T:X \to X$ is a
...
\end{defn}
        ...
        \begin{thm}[Contraction Mapping Theorem] A strict contraction on a
...
is left as an exercise because I don't feel like typing).  \end{proof}
    ...
    \begin{defn}[Normed Vector Space] A normed vector space $X$ is a vector
    ...
    \begin{defn}[Banach Space] A Banach space is a complete normed vector
    space.  \end{defn}
...
support ``escaping to $\infty$.'' \end{exm}
    ...
    \begin{defn}[Sobolev Space] Define the Sobolev space to be:
    ...
    \begin{thm} $L^p(X,\mu)$ is a Banach space.  \begin{proof} Non-negativity
        ...
        \begin{mdframed}[style=default] \begin{thm}[Jensen's Inequality] Define
            ...
            Cauchy-Schwarz: \begin{mdframed}[style=default]
                \begin{thm}[Cauchy-Schwarz] If $f,g$ are square-integrable,
            ...
            \begin{mdframed}[style=default] \begin{thm}[H\"older's Inequality]
            ...
            \begin{mdframed}[style=default] \begin{thm}[Minkowski's inequality]
    ...
    \begin{defn}[Schauder basis] $X$ has a Schauder basis if it has a countable
...
by $\sum c_nx_n$.  \end{defn}
    ...
    Consider a linear operator $T$.  \begin{defn}[Bounded Linear Operator] $T$
...
\end{description} \end{exm}
    ...
    \begin{thm}[BLT] If $X$ is a normed vector space, $Y$ Banach, and $M$ a
        ...
        \begin{proof} Clearly we define $\overline{T}$ to be the limit
            ...
            \begin{defn}[Linear Functional] A linear functional on $X$ is a
    ...
    \begin{thm}[Rank-Nullity] For finite dimensional vector spaces $\R^n$, we
...
$\mathrm{ker}\ T \perp \mathrm{im}\ T^t$.  \end{thm}
    ...
    \begin{thm}[Open mapping theorem] A bounded surjective map is open.
...
\end{exm}
    ...
    \begin{defn}[Weak convergence] We weaken this notion further by declaring
    ...
    \begin{defn}[Weak convergence in $Y$] $y_k \rightharpoonup y$ weakly in $Y$
...
analogous to convergence in measure.} \end{defn}
    ...
    \begin{thm}[Hahn-Banach] Let $X$ be a vector space and $p: X \to \R$
...
\end{thm}
    ...
    \begin{defn}[Inner Product] An inner product in a complex vector space is a
    ...
    \begin{defn}[Hilbert Space] A Hilbert space is a complete inner product
    space.  \end{defn}
...
not Hilbert in general. We simply take the completion.  \end{exm}
    ...
    \begin{thm}[Cauchy-Schwarz] For all $x,y \in X$, we have $\abs{(x,y)} \leq
    ...
    \begin{thm}[Parallelogram Law] In any inner product space, $\norm{a+b}^2 +
    \norm{a-b}^2 = 2(\norm{a}^2 + \norm{b}^2)$.  \end{thm}
...
However, we see $\norm{x-y}^2 = \norm{x-y'^2} + \norm{y-y'}^2$ and thus $y=y'$.
    ...
    \begin{defn}[Orthogonal Subset] $U$ is an orthogonal subset if $u \perp v$
    for all $u,v \in U$.  \end{defn}

    \begin{defn}[Orthonormal Subset] $U$ is orthonormal if $U$ is orthogonal
...
basis.  \end{exm}
    ...
    \begin{defn}[Unconditional Convergence] In a Banach space, we'll say the
...
$\norm{\sum_{u_{\alpha},x}u_{\alpha}}^2$ converges.
    ...
    \begin{thm}[Riesz Representation Theorem] Let $H$ be a Hilbert space. Then
...
\end{lem}
    ...
    \begin{thm}[Spectral Theorem, compact case] Let $A: H \to H$ be compact and
...
\end{enumerate} \end{prop}
    ...
    \begin{defn}[Normal Operator] An operator is normal if it commutes with its
    adjoint.  \end{defn}
...
\end{thm}
    ...
    \begin{defn}[Fourier Transform] Let $u \in L^1(\R^d)$. Then the Fourier
...
compactly supported smooth function.  \end{defn}
    ...
    \begin{defn}[Strong Topology] We declare $\phi_j \to 0$ in the strong
...
D^{\alpha}\phi(a)$.  \end{enumerate} \end{exm}
    ...
    We can define this for any distribution.  \begin{defn}[Weak Derivative] Let
...
\end{exm}
    ...
    \begin{defn}[Convolution of Distributions] The convolution of two
